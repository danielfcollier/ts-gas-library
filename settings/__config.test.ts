...
function qunit(qunitParams: QUnitParams) {
    ...
    QUnit.testDone(details => {
        const result = `## Test ${details.name} > Passed: ${details.passed} `
            + `> Failed: ${details.failed} > Average Runtime: ${Math.round(details.runtime / details.total)} ms`;
    ...
    QUnit.done(details => {
        ...
        console.log(
            ...
            + `> Failed: ${details.failed} > Runtime: ${Math.round((details.runtime / 1000))} seconds`
        );
    })
    // ----------------------------------------------------------------------------------------------
}
