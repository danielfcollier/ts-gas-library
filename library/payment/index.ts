...
export class Customer {
    // ----------------------------------------------------------------------------------------------
    private static API: string = process.env.PAYMENT_ROOT_URL;
    // ----------------------------------------------------------------------------------------------
    static create(customer: Omit<ICustomer, 'id'>, options?) {
        const response = FetchApp.post(`${this.API}/customers`, customer, options);
        return this.reponseDTO(response, customer);
    }
    // ----------------------------------------------------------------------------------------------
    static read(customer: Pick<ICustomer, 'id'>, options?) {
        const { id } = customer;
        const response = FetchApp.get(`${this.API}/customers/${id}`, options);
        return response;
    }
    // ----------------------------------------------------------------------------------------------
    static update(customer: ICustomer, options?) {
        const { id } = customer;
        const response = FetchApp.post(`${this.API}/customers/${id}`, customer, options);
        return this.reponseDTO(response, customer);
    }
    // ----------------------------------------------------------------------------------------------
    static remove(customer: Pick<ICustomer, 'id'>, options?) {
        const { id } = customer;
        const { deleted } = FetchApp.remove(`${this.API}/customers/${id}`, options);
        return deleted;
    }
    // ----------------------------------------------------------------------------------------------
...
export class Order {
    ...
    static create(order: Omit<IOrder, 'id'>, options?) {
        const response = FetchApp.post(`${this.API}/payments`, order, options);
    ...
    static read(order: Pick<IOrder, 'id' | 'installmentCount'>, options?) {
        ...
        const response = FetchApp.get(`${this.API}/${endpoint}/${id}`, options);
    ...
    static cancel(order: Pick<IOrder, 'id' | 'installmentCount'>, options?) {
        const { id } = order;
        const endpoint = this.determinePaymentEndpoint(order);
        const { deleted } = FetchApp.remove(`${this.API}/${endpoint}/${id}`, options);
        return deleted;
    }
    // ----------------------------------------------------------------------------------------------
    static refund(order: Pick<IOrder, 'id' | 'installmentCount' | 'billingType' | 'status' | 'paymentDate'>, options?) {
        ...
        const { id } = order;
        const endpoint = this.determinePaymentEndpoint(order);
        const response = FetchApp.post(`${this.API}/${endpoint}/${id}/refund`, {}, options);
        return this.responseDTO(response, order, options);
    }
    // ----------------------------------------------------------------------------------------------
    static confirmCash(order: Pick<IOrder, 'id' | 'value'>, paymentDate: Date, options?) {
        ...
        const response = FetchApp.post(`${this.API}/payments/${id}/receiveInCash`, payload, options);
        return this.responseDTO(response, order, options);
    }
    // ----------------------------------------------------------------------------------------------
    static unconfirmCash(order: Pick<IOrder, 'id'>, options?) {
        const { id } = order;
        const response = FetchApp.post(`${this.API}/payments/${id}/undoReceivedInCash`, {}, options);
        return this.responseDTO(response, order, options);
    }
    // ----------------------------------------------------------------------------------------------
    static getBarCode(order: Pick<IOrder, 'id'>, options?) {
        const { id } = order;
        const { identificationField } = FetchApp.get(`${this.API}/payments/${id}/identificationField`, options);
        return identificationField;
    }
    // ----------------------------------------------------------------------------------------------
    static getQRCode(order: Pick<IOrder, 'id'>, options?) {
        const { id } = order;
        const qrCode = FetchApp.get(`${this.API}/payments/${id}/pixQrCode`, options);
    ...
    private static getInstallments(order: Pick<IOrder, 'id'>, options?) {
        const { id } = order;
        const { totalCount, data } = FetchApp.get(`${this.API}/payments?installment=${id}`, options);
...
export class Finances {
    ...
    static doBankTransfer(transferInfo, options?) {
        return FetchApp.post(`${this.API}/transfers`, transferInfo, options);
    }
    // ----------------------------------------------------------------------------------------------
    static getCurrentBalance(options?) {
        const { totalBalance } = FetchApp.get(`${this.API}/finance/getCurrentBalance`, options);
    ...
    private static getOrdersPerQuery(orders: Partial<IOrder>[], queryParams: OrdersQuery) {
        ...
        const response = FetchApp.get(`${this.API}/payments/?${queryString}`);
